...
function ProfileContent() {
  ...
  const navItems = [
    { key: 'gioi-thieu', label: 'Cá nhân', icon: <div className="w-6 h-6 rounded-full bg-gray-200 overflow-hidden relative mr-3">{profile?.urlAnhDaiDien ? <Image src={getValidSrc(profile.urlAnhDaiDien)} alt="P" fill className="object-cover" sizes="24px" /> : <span className="flex items-center justify-center h-full text-[10px] font-bold">{firstName.charAt(0)}</span>}</div> },
          ...
          <div className={`${activeTab !== 'menu' && activeTab !== '' ? 'hidden md:block' : 'block'} md:hidden`}>
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="w-12 h-12 rounded-full bg-gray-200 overflow-hidden relative">
                <Image src={getValidSrc(profile?.urlAnhDaiDien)} alt="P" fill className="object-cover" />
              </div>
            </div>
                      ...
                      <div className="relative flex-shrink-0">
                        <div className="w-28 h-28 rounded-full overflow-hidden relative shadow-xl" style={{ background: 'linear-gradient(135deg, #FF385C 0%, #BD1E59 50%, #6F019C 100%)', padding: '3px' }}>
                          <div className="w-full h-full rounded-full overflow-hidden bg-white relative">
                            <Image src={getValidSrc(profile?.urlAnhDaiDien)} alt="P" fill className="object-cover" sizes="112px" />
                          </div>
                        </div>
